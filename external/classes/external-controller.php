...
class ExternalController
{
    ...
    private function check($stud): void
    {
        if (!$stud->is_valid()) {
    ...
    function exit_with_error($message): void
    {
    ...
    function authReg(): void
    {
        ...
        if (require_birthday) {
            ...
            if ($stud->get_bday()!=$bday){
                exit_with_error('Geburtsdatum nicht korrekt. Korrigieren Sie Ihre Eingabe und wenden Sie sich ggf. an unser Sekretariat.');
            }
    ...
    function deployAuth(): void
    {
        ...
        if ($this->f3->get('PARAMS.hash')==hash_hmac('sha256', $stud->get_bday(), 'bday-transfer'))
            $this->deploy_pass($this->f3, $this->f3->get('PARAMS.id'), $this->f3->get('PARAMS.deploy'));
        exit_with_error('Fehlende Authentifizierung. Bitte scannen Sie den QR-Code erneut.');
    }
    ...
    function deploy_pass($f3, $id, $deploy): void
    {
        ...
        switch ($deploy) {
            ...
            default:
                // unknown deploy type        
                exit_with_error('Fehler: Unbekannter Wallet-Typ oder Ausweis im gewählten Format nicht verfügbar.');
                break;
            }    
    }
}
